...
const DEFAULT = {
  ...
  hero_subtitle: "Vi definierar premiumstädning genom certifierad expertis och hållbara metoder. Från fönsterputs till flyttstädning, vi tar hand om detaljerna.",
...
export const Hero = () => {
  ...
  React.useEffect(() => {
    if (raw.primary_color) document.documentElement.style.setProperty("--color-primary", raw.primary_color);
    if (raw.secondary_color) document.documentElement.style.setProperty("--color-secondary", raw.secondary_color);
  }, [raw.primary_color, raw.secondary_color]);
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          ...
          <div className="mt-9 flex flex-col sm:flex-row gap-4">
            ...
            <a href={`tel:${ws.phone?.replace(/[^0-9+]/g,"")}`} data-testid="hero-call-btn"
              ...
              <Phone size={18} /> {ws.phone}
            </a>
          </div>
          <div className="mt-10 flex items-center gap-6 text-sm text-slate-500">
            <span className="inline-flex items-center gap-2">
              <ShieldCheck size={16} className="text-[#166534]" /> {ws.badge1}
            </span>
            <span className="inline-flex items-center gap-2">
              <Leaf size={16} className="text-[#166534]" /> {ws.badge2}
            </span>
          </div>
        </motion.div>
        {/* Right */}
        ...
        >
          <div className="relative rounded-[2rem] overflow-hidden shadow-[0_30px_80px_-30px_rgba(20,20,20,0.4)]">
            <img src={ws.hero_image || DEFAULT.hero_image}
              alt="Professionell städning i Umeå av PureNorth Städ"
